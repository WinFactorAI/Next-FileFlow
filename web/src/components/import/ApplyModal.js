...
const ApplyModal = ({
    ...
    const setSelectFileTime = () => {
        fileTimer = setInterval(() => {
            ...
            if (id) {
                applyApi.getSelectFileInfo(id).then(response => {
                    let data = response;
    ...
    const handleSelectFile = () => {
        ...
        applyApi.getImportDevice().then(response => {
            ...
            if (devices.length === 1 || form.getFieldValue('deviceKey') !== "") {
                ...
                applyApi.selectFile({ 'deviceKey': devices[0].id }).then(response => {
    ...
    const submitAntiVirus = () => {
        ...
        applyApi.antiVirus(params).then(response => {
            setRedTip(false);
    ...
    const submitUpload = () => {
        ...
        applyApi.uploadFile(form.getFieldsValue()).then(response => {
