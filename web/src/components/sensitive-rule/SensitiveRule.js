...
import { Button, Layout, notification, Popconfirm, Popover, Switch, Upload } from "antd";
import React, { useState } from 'react';
import sensitiveRuleApi from "../../api/sensitive-rule";
import Show from "../../dd/fi/show";
import ColumnState, { useColumnState } from "../../hook/column-state";
import ChangeOwner from "../asset/ChangeOwner";
import SensitiveRuleDrawer from "./SensitiveRuleDrawer";
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (text, record, _, action) => [
                // <Show menu={'sensitive-rule-exec'} key={'sensitive-rule-exec'}>
                //     <a
                //         key="run"
                //         onClick={() => {
    ...
    return (<Content className="page-container">
        <ProTable
            ...
            toolBarRender={() => [
                ...
                <Show menu={'sensitive-rule-add'}>
                    <Button key="button" type="primary" onClick={() => {
                        setVisibleSensitiveRuleDrawer(true)
                    }}>
                        规则模型校验器
                    </Button>
                </Show>,
        ...
        <SensitiveRuleDrawer
            id={selectedRowKey}
            visible={visibleSensitiveRuleDrawer}
            confirmLoading={confirmLoading}
            handleCancel={() => {
                setVisibleSensitiveRuleDrawer(false);
                setSelectedRowKey(undefined);
            }}
            handleOk={async (values) => {
                setConfirmLoading(true);
