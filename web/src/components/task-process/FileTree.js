  ...
  useEffect(() => {
    ...
    const findFirstValidNode = (nodes) => {
      for (const node of nodes) {
        ...
        if (node.children) {
          const childResult = findFirstValidNode(node.children);
    ...
    const firstValidValue = formattedData.length > 0
      ? findFirstValidNode(formattedData)
      : null;

    setSelectedPath(firstValidValue);
  }, [data]);
