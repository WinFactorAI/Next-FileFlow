...
class FileSystemSpace extends Component {
    ...
    componentDidMount() {
        ...
        this.setState({
            ...
            currentPathCallback: this.props.currentPathCallback,
    ...
    componentDidUpdate(prevProps) {
        ...
        const propsChanged = this.watchedStateKeys.some(prop =>
    ...
    loadFiles = async (key) => {
        ...
        try {
            ...
            let result = await request.post(`/worker/${this.state.storageType}/${this.state.storageId}/ls`, formData);
            if (result['code'] !== 1) {
            ...
            // console.log(" key ",key);
            if(this.state.currentPathCallback)
                this.state.currentPathCallback(key)
    ...
    uploadFile = (file, dir, callback) => {
        ...
        let url = `${server}/worker/${this.state.storageType}/${this.state.storageId}/upload?X-Auth-Token=${getToken()}&dir=${dir}`
    ...
    delete = async (key) => {
        ...
        let result = await request.post(`/worker/${this.state.storageType}/${this.state.storageId}/rm`, formData);
        if (result['code'] !== 1) {
            message.error(result['message']);
        }
    }
    ...
    showEditor = async (name, key) => {
        ...
        let fileContent = await request.get(`${server}/worker/${this.state.storageType}/${this.state.storageId}/download?file=${window.encodeURIComponent(key)}&t=${new Date().getTime()}`);
        this.setState({
            currentFileKey: key,
            fileName: name,
    ...
    edit = async () => {
        ...
        let url = `${server}/worker/${this.state.storageType}/${this.state.storageId}/edit`
        let formData = new FormData();
    ...
    render() {
        ...
        return (
            <div>
                <Card title={title} bordered={true} size="small" className="fileCard" style={{ minHeight: this.state.minHeight}}>
                    ...
                    <Table columns={columns}
                        ...
                        style={{ 
                            overflow: "auto",
                            minWidth: '1000px', // 关键属性
                            tableLayout: 'fixed' // 防止自动调整列宽
                          }}
                        onRow={record => {
                            return {
                                onDoubleClick: event => {
                            ...
                            onOk={() => {
                                this.mkdirFormRef.current
                                    ...
                                    .then(async values => {
                                        ...
                                        let result = await request.post(`/worker/${this.state.storageType}/${this.state.storageId}/mkdir?${paramStr}`);
                                        if (result.code === 1) {
                                ...
                                this.renameFormRef.current
                                    ...
                                    .then(async values => {
                                        ...
                                        try {
                                            ...
                                            let result = await request.post(`/worker/${this.state.storageType}/${this.state.storageId}/rename?${paramStr}`);
                                            if (result['code'] === 1) {
                                                message.success('重命名成功');
                                                this.refresh();
                                            } else {
...
export default FileSystemSpace;