...
import { Button, Col, Collapse, ConfigProvider, Input, Layout, message, Modal, Popconfirm, Row, Select, Switch, Table } from "antd";
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (text, record, _, action) => [
                ...
                <Show menu={'user-break'} key={'user-break'} >
                    <Popconfirm
                        className={record.online && record.id !== getCurrentUser()['id'] ? '' : 'hidden'}
                        key={'confirm-break'}
                        title="您确认要断开此用户吗?"
                        onConfirm={async () => {
                            await accountApi.breakLogoutById(record.id);
                            actionRef.current.reload();
                        }}
                        okText="确认"
                        cancelText="取消"
                    >
                        <a key='break' className='danger'>断开</a>
                    </Popconfirm>
                </Show>,
                <TableDropdown
                    key="actionGroup"
                    onSelect={(key) => {
                        switch (key) {
                            ...
                            case 'user-login-policy':
                                ...
                                break;
                            case 'user-strategy-policy':
                                navigate(`/user/${record['id']}?activeKey=strategy-policy`);
                                break;
                        }
                    }}
                    menus={[
                        ...
                        { key: 'user-strategy-policy', name: '空间策略', disabled: !hasMenu('user-strategy-policy') },
                    ]}
                />,
            ],
        },
