...
import workDistDataApi from "../../../api/worker/dict-data";
import request from "../../../common/request";
import ColumnState, { useColumnState } from "../../../hook/column-state";
import DeviceSelect from "../../export/DeviceSelect";
import TaskProgress from "../../export/TaskProgress";
...
const Apply = () => {
    ...
    const setBurnTime = (id) => {
        const burnTimer = setInterval(() => {
            workerApplyApi.getById(id).then(response => {
    ...
    const burnRun = (row) => {
        workerApplyApi.burn(row).then(response => {
            message.info("开始刻录光盘");
    ...
    const handleBurn = (row) => {
        workerApplyApi.getImportDevice().then(response => {
    ...
    useEffect(() => {
        workDistDataApi.list("apply_status").then(res => {
            setApplyStatusOptions(res)
        })
        workDistDataApi.list("task_process_status").then(res => {
            setTaskProcessStatusOptions(res)
        })
        workDistDataApi.list("disc_status").then(res => {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (text, record, _, action) => [
                // menu={'export-apply-edit'}
                // <Show key={'export-apply-edit'}>
                <a
                    key="edit"
                    onClick={() => {
                        setVisible(true);
